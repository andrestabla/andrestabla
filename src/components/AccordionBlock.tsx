...
export default function AccordionBlock({ data }: { data: any }) {
    ...
    return (
        <div className="w-full flex flex-col gap-4">
            {title && <h3 className="text-2xl font-bold text-slate-900 tracking-tight mb-2">{title}</h3>}
            {items.map((item: any, i: number) => (
                <div key={i} className="border border-slate-200 rounded-xl overflow-hidden bg-white shadow-sm hover:shadow-md transition-all">
                    ...
                    >
                        <span className="font-semibold text-slate-800">{item.question || 'Pregunta...'}</span>
                        {openIndex === i ? <ChevronUp size={20} className="text-indigo-500" /> : <ChevronDown size={20} className="text-slate-400" />}
                    </button>
                    {openIndex === i && (
                        <div className="p-5 text-slate-600 leading-relaxed border-t border-slate-100 bg-white animate-in slide-in-from-top-2">
